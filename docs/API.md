...
| `GET/POST /api/admin/menu/items`, `PATCH/DELETE …/[id]` | S (BRANCH_MANAGER) | Item CRUD with nested `variants`, `addOns`, per-branch `branchOverrides` (priceOverride, `variantPrices` e.g. `{"Half":70,"Full":120}`, available, stockQty, time window). Portions keep their ids across saves. `GET ?format=csv` exports in the import format. DELETE = soft-deactivate. Price changes audited. |
| `PATCH /api/admin/menu/prices` | S (BRANCH_MANAGER) | `{changes:[{menuItemId, branchId, price? , variantPrices?}]}` — the Menu page's "Edit prices" grid. Prices only; availability and stock untouched. Audited. |
| `POST /api/admin/menu/import` | S (BRANCH_MANAGER) | CSV body, format in `src/lib/menu-csv.ts`: `id,category,name,name_hindi,description,veg,spicy,bestseller,recommended,emoji,<branch>_half,<branch>_full,<branch>_price,…,addons`. `?dryRun=1` previews (new / updated / renamed / not in file / errors); `?hideMissing=1` soft-hides dishes the file omits. All-or-nothing; a file with errors changes nothing. |
